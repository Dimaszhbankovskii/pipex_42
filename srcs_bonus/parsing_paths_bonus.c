...
static char	**parsing_paths(t_pipexb *pipex)
{
	...
	if (!tmp1)
		end_program(ERROR_NO_PATHS_IN_ENVP, pipex, 0, 0);
	tmp2 = ft_strdup(tmp1 + ft_strlen("PATH="));
	if (!tmp2)
		end_program(ERROR_PARSING_PATHS, pipex, 0, 0);
	paths = ft_split(tmp2, ':');
	if (!paths)
	{
		free(tmp2);
...
void	create_paths(t_pipexb *pipex)
{
	...
	if (!pipex->path_cmd)
		end_program(ERROR_INIT_PATH_CMD, pipex, 0, 0);
	i = 0;
	while (i < pipex->num_cmds)
	{
		pipex->path_cmd[i] = search_paths(pipex, pipex->cmds[i][0]);
		if (!pipex->path_cmd[i])
			end_program(ERROR_NO_PATH_CMD, pipex, 0, 0);
		i++;
	}
